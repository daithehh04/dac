    ...
    <footer className='footer bg-[#000D10] text-[#888] md:pt-[4.98rem] pt-[11.47rem] max-md:h-auto'>
      <div className="content max-md:flex flex-col justify-center ">
        ...
        <div className='flex items-start max-md:flex-col justify-between md:mt-[3.58rem] mt-[6.4rem] description2 md:!tracking-[-0.05rem] !text-[#888]'>
          <div className="left md:w-[44rem] md:mr-[10.4rem] max-md:mb-[8rem] title_company_name" dangerouslySetInnerHTML={{ __html: `${data?.homepage?.footer?.contentColumn1}` }}>
          </div>
          <div ref={contentRef} className="right title_company_name" dangerouslySetInnerHTML={{ __html: `${data?.homepage?.footer?.contentColumn2}` }}>
          </div>
        </div>
