            ...
            >
              {({ isActive }) => (
                <Link href={`/${lang}/san-pham-dich-vu/${item?.product_category?.info?.featureProduct}`} className='relative w-full h-full '>
                  ...
                  <h3 className='absolute md:bottom-[4.64rem] text_product_item_home bottom-[10rem] md:pr-[2.9rem] w-full md:pl-[2.64rem] max-md:left-[4.27rem] max-md:right-[4.27rem] text-[6.93333rem]  md:text-[2.5rem] font-bold text-white leading-[1.16]' dangerouslySetInnerHTML={{ __html: `${item.product_category?.info?.title}` }} ></h3>
                </Link>
              )}
            </SwiperSlide>
          )
