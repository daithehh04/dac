          ...
          return (
            ...
            >
              {({ isActive }) => (
                ...
                >
                  <Image src={item?.background?.sourceUrl || imgVideo} width={1000} height={1000} quality={100} alt='img' className={`w-full h-full absolute z-[1] inset-0 object-cover ${isPlay ? 'hidden' : ''}`} />
                  <svg xmlns="http://www.w3.org/2000/svg" className={`md:w-[4.5rem] shadow-md md:h-[5rem] w-[10rem] h-[10rem] absolute z-[1] top-[50%] left-[50%] -translate-x-1/2 -translat-y-1/2 cursor-pointer ${isPlay ? 'hidden' : 'block'}`} viewBox="0 0 64 76" fill="none">
