        ...
        <section className='md:pt-[5.05rem] md:px-[12.03rem] px-[4.27rem] md:pb-[6.35rem]'>
            <div className='flex max-md:flex-col'>
                ...
                <div className='md:mr-[2.6rem]'>
                    ...
                    <div
                        className='md:w-[36.66667rem] description !font-normal text-justify md:mt-[1.77rem] mt-[6.13rem]'
                ...
                <div className='flex flex-col max-md:pb-[4rem]'>
                    {dataSystem?.gallery?.map((item, index) => (
                        <div key={index}>
                            <Image src={item?.sourceUrl || img1} alt={item?.altText || 'img'} quality={100} width={1000} height={1000} className={`md:w-[36.66667rem] h-[55rem] md:h-[22.1875rem] object-cover md:mb-[1.93rem] ${index === 0 ? 'max-md:hidden' : ''}`} />
                        </div>
                    ))}
                </div>
            </div>
